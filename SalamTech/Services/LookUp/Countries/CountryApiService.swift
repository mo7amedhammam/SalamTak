...
final class GetCountriesApiService{

static func GetCountries( completion: @escaping ( Bool , ModelCountries?, String?) -> ()) {
    
    let url = URLs().GetCountries
        let header:HTTPHeaders = ["Authorization":Helper.getAccessToken()]
    
    AF.request(url, method: .get,parameters: nil ,encoding: JSONEncoding.default ,headers: header )
        .validate(statusCode: 200..<500)
        .responseDecodable(completionHandler: { ( response : DataResponse<ModelCountries?, AFError>) in
