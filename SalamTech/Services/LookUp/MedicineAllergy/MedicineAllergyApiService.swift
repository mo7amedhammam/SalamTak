...
final class GetMedicineAllergyApiService{

static func GetMedicineAllergy( completion: @escaping ( Bool , ModelMedicineAllergy?, String?) -> ()) {
    
    let url = URLs().GetMedicineAllergy
        let header:HTTPHeaders = ["Authorization":Helper.getAccessToken()]
    
    AF.request(url, method: .get,parameters: nil ,encoding: JSONEncoding.default ,headers: header )
        .validate(statusCode: 200..<500)
        .responseDecodable(completionHandler: { ( response : DataResponse<ModelMedicineAllergy?, AFError>) in
