...
//@ObservedObject private var RegisterVM = ViewModelRegister()
...
//                    .frame( height: 55, alignment: .center)
                ...
                Text("Code \n\(passedmodel?.publishedUserResetModel?.data?.code ?? 0)").font(.custom("SF UI Text", size: 16)).fontWeight(.light)
...
//                            print(passedmodel?.publishedUserRegisteredModel?.Message ?? "")
                            ...
                            if checkOTP(sentOTP: passedmodel?.publishedUserResetModel?.data?.code ?? 1111, TypedOTP: Int(otp) ?? 0000){
                                self.matchedOTP.toggle()
                                self.userId = passedmodel?.publishedUserResetModel?.data?.userId ?? 0
                                // Create User
